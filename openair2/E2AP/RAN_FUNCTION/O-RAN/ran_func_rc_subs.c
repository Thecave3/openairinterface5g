...
static bool eq_ric_id(const void *value, const void *it)
{
  const ric_gen_id_t *ric_id = (const ric_gen_id_t *)value;
  const ran_param_data_t *dit = (const ran_param_data_t *)it;
  return eq_ric_gen_id(ric_id, &dit->ric_id);
}
...
{
  pthread_mutex_lock(&rc_mutex);
  /* find the sequence element that matches the RIC ID */
  elm_arr_t elm_rs1_param3 = find_if(&rc_subs_data->rs1_param3, (void *)&ric_id, eq_ric_id);
  ...
  /* find the sequence element that matches the RIC ID */
  elm_arr_t elm_rs1_param4 = find_if(&rc_subs_data->rs1_param4, (void *)&ric_id, eq_ric_id);
  ...
  /* find the sequence element that matches the RIC ID */
  elm_arr_t elm_rs4_param202 = find_if(&rc_subs_data->rs4_param202, (void *)&ric_id, eq_ric_id);
